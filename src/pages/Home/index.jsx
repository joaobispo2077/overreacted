...
const Home = () => {
  ...
  const handleFilterPerTitle = (event) => {
    ...
    const postsFilteredByTitle = allPosts.filter((post) =>
      post.title.toLowerCase().includes(searchingTitle.toLowerCase()),
    );

    const filteredPostBySearchingTitle = postsFilteredByTitle
  ...
  return (
    <section className="container">
      <div className="search-container">
        {!!searchTitle && <h1>Searching: {searchTitle}</h1>}
      ...
      {filteredPost.length === 0 && <p>Post não encontrado</p>}
